/**
 * Wave 6 — replace numeric borderRadius with semantic CSS vars
 * in Career Jobs + Workforce Ops (Shift Jobs already Wave 4).
 * Mapping: pill 999; employer≥24; employee 20–22; chip 13–18; button 12; 10/8 ladder.
 * Circles ("50%") left untouched (not matched by numeric patterns).
 */
import fs from "fs";
import path from "path";

const roots = [
  "src/features/employer/careerJobs",
  "src/features/employee/careerJobs",
  "src/features/employer/workforceOps",
  "src/features/employee/workforceOps",
];
...
for (const root of roots) {
  for (const file of walk(root)) {
    ...
    const after = before
      .replace(/borderRadius:\s*(\d+)\b/g, (_m, num) => {
        replacements += 1;
