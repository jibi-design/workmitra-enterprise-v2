...
function sessionPepper(): string {
  try {
    const fromEnv = (import.meta.env.VITE_DOC_ACCESS_SESSION_PEPPER as string | undefined)?.trim();
...
/** Minimal SHA-256 (sync) for HMAC — browser Web Crypto is async-only. */
...
function hmacSha256Hex(keyMaterial: string, message: string): string {
  ...
  const inner = sha256Bytes(new Uint8Array([...ipad, ...enc.encode(message)]));
  return toHex(sha256Bytes(new Uint8Array([...opad, ...inner])));
}
...
export function signDocAccessSession(payload: DocAccessSessionSignPayload): string {
  ...
  return hmacSha256Hex(keyMaterial, canonicalizeDocAccessSessionPayload(payload));
}
