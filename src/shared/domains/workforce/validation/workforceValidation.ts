...
// File: workforceValidation.ts
// Path: C:\projects\WorkMitra_Enterprise_v2\src\shared\domains\workforce\validation\workforceValidation.ts
...
export function validateCategoryName(
  ...
  const duplicate = existing.find(
    (category) => category.name.toLowerCase() === trimmed.toLowerCase() && category.id !== editId,
  );
...
export function validateAddStaff(
  ...
  const duplicate = existingStaff.find(
    (staff) => staff.employeeUniqueId === trimmedId && staff.status === "active",
  );
...
export function validateShifts(shifts: AnnouncementShift[]): ValidationResult {
  ...
  const names = shifts.map((shift) => shift.name.trim().toLowerCase());
...
export function validateAnnouncementStep2(
  ...
  for (const categoryId of targetCategories) {
    const shiftMap = vacancyMap[categoryId];
...
export function checkDateConflict(
  ...
  return confirmedAnnouncements.some(
    (announcement) => announcement.date === eventDate && announcement.status === "confirmed",
  );
}
