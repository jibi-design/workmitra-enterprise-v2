...
export function DemandPlannerStep3({ step1, slots, isSubmitting, onSubmit, onBack }: Props) {
  ...
  const payLabel =
    uniquePays.length === 1
      ? `${uniquePays[0]}/day`
      : `${Math.min(...uniquePays)}–${Math.max(...uniquePays)}/day`;
    ...
    <div>
      ...
      >
        ...
        <div style={{ display: "grid", gap: 4, fontSize: 12, color: "var(--wm-er-text)" }}>
          <div>
            <span style={{ color: "var(--wm-er-muted)" }}>Company:</span> {step1.companyName || "—"}
          </div>
          <div>
            <span style={{ color: "var(--wm-er-muted)" }}>Location:</span> {step1.locationName}
          </div>
          <div>
            <span style={{ color: "var(--wm-er-muted)" }}>Category:</span> {step1.category}
          </div>
          <div>
            <span style={{ color: "var(--wm-er-muted)" }}>Dates:</span>{" "}
            {fmtPlanDate(step1.startDate)} &rarr; {fmtPlanDate(step1.endDate)}
          </div>
        </div>
      </div>
      ...
      >
        {[
          ...
          >
            <div style={{ fontSize: 11, color: "var(--wm-er-muted)", marginBottom: 2 }}>
              {s.label}
            </div>
          ...
          {slots.map((slot, idx) => (
            ...
            >
              ...
              <div style={{ fontSize: 12, color: "var(--wm-er-muted)" }}>
                <b style={{ color: "var(--wm-er-text)" }}>{slot.workers}</b> workers &middot;{" "}
                <b style={{ color: "var(--wm-er-text)" }}>{slot.payPerDay}</b>/day
                <span
                  style={{
                    marginLeft: 8,
                    color: "var(--wm-planner-accent-strong)",
      ...
      >
        <div style={{ fontWeight: 700, color: "var(--wm-er-text)", marginBottom: 4 }}>
          What happens when you submit:
        </div>
        <div>
          &#10003; {totalDays} shift {totalDays === 1 ? "post is" : "posts are"} created
          automatically
        </div>
