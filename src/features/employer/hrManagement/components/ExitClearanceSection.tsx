...
export function ExitClearanceSection({ record }: Props) {
  ...
  const handleFinalize = () => {
    ...
    const result = hrManagementStorage.completeExitSaga(record.id);

    if (!result.ok) {
      if (result.reason === "not_ready") {
        setFinalizeError(
          "Complete all clearance items and send the experience letter before finalizing.",
        );
      } else if (result.reason === "staff_write_error") {
        setFinalizeError("Could not update My Staff. Exit was rolled back. Please try again.");
      } else if (result.reason === "lifecycle_write_error") {
        setFinalizeError(
          "Could not update employment history. Exit was rolled back. Please try again.",
        );
      } else if (result.reason === "shared_employment_write_error") {
        setFinalizeError(
          "Could not update career employment records. Exit was rolled back. Please try again.",
        );
      } else if (result.reason === "hr_write_error") {
        setFinalizeError("Could not save HR exit status. Please try again.");
      } else {
        setFinalizeError("Could not finalize this exit. Please try again.");
      }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {exitData.noticePeriod.waived ? (
            ...
            <>
              <div style={{ fontSize: 12, fontWeight: 700, color: "var(--wm-er-text)" }}>
                {exitData.noticePeriod.totalDays} days ({fmtDate(exitData.noticePeriod.startDate)} –{" "}
                {fmtDate(exitData.noticePeriod.endDate)})
              </div>
              <div
                style={{
                  fontSize: 11,
      ...
      >
        ...
        <div
          style={{ fontSize: 11, color: "var(--wm-er-muted)", marginBottom: 8, lineHeight: 1.5 }}
        >
      ...
      <div
        style={{
          ...
          background: "#fff",
          borderRadius: 12,
          border: `1px solid ${exitData.experienceLetterSent ? "rgba(22,163,74,0.2)" : "var(--wm-er-border, #e5e7eb)"}`,
        }}
      >
        <div style={{ fontWeight: 900, fontSize: 14, color: "var(--wm-er-text)", marginBottom: 8 }}>
          Experience Letter
      ...
      {exitData.exitCompletedAt || record.status === "exited" ? (
        <div
          style={{
            padding: 16,
        ...
        >
          ...
          <div style={{ fontSize: 12, color: "var(--wm-er-muted)", marginTop: 4 }}>
            {exitData.exitCompletedAt
              ? `Finalized on ${fmtDate(exitData.exitCompletedAt)}`
              : "Exit finalized"}
          </div>
        </div>
      ) : (
        ...
        >
          {!showConfirmFinalize ? (
            ...
            <div>
              ...
              >
                This action cannot be undone. The employee's record will be marked as exited and
                their Work Vault will be updated.
              </div>
