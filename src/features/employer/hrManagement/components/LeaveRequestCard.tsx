...
export function LeaveRequestCard({ request, mode }: Props) {
  ...
  const handleApprove = async () => {
    await hrService.updateLeaveStatus(request.id, "approved");
  };

  const handleReject = async () => {
    await hrService.updateLeaveStatus(
      request.id,
      "rejected",
      rejectComment.trim() || "No reason provided",
    );
  ...
  const handleCancel = async () => {
    await hrService.updateLeaveStatus(request.id, "cancelled");
  };
  ...
  return (
    <>
            ...
            {mode === "employer" && (
              <>
                <button
                  ...
                  style={{ fontSize: 11, padding: "6px 14px", background: "#16a34a" }}
                >
                  Approve
                </button>
                <button
      ...
      <CenterModal
        ...
        onBackdropClose={() => setShowRejectModal(false)}
      ...
      >
        <div style={{ padding: 20 }}>
          ...
          <div
            style={{ fontSize: 12, color: "var(--wm-er-muted)", marginTop: 4, marginBottom: 14 }}
          >
            {request.employeeName} — {LEAVE_TYPE_LABELS[request.leaveType]} ({request.totalDays}{" "}
            days)
          </div>
          <label
            style={{
          ...
          <div style={{ marginTop: 14, display: "flex", justifyContent: "flex-end", gap: 10 }}>
            ...
            <button
              ...
              style={{ background: "#dc2626", opacity: rejectComment.trim().length > 0 ? 1 : 0.5 }}
            >
              Reject
            </button>
          </div>
