...
import {
  STATUS_TABS,
  getHrGuideDismissedKey,
  matchesSearch,
  matchesFilter,
} from "../helpers/hrPageHelpers";
...
export function HRManagementPage() {
  ...
  const handleDismissGuide = useCallback(() => {
    localStorage.setItem(getHrGuideDismissedKey(), "1");
  ...
  const showFilters =
    activeTab === "active" && allRecords.filter((r) => r.status === "active").length >= 2;
    ...
    <div data-testid="employer-hr-management-page">
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 12 }}>
          <HRNavItem
            label="My Staff Directory"
            description="View all staff, categories, and exit processing"
            onClick={() => nav(ROUTE_PATHS.employerMyStaff)}
          />
          <HRNavItem
            label="Verify Employee"
            description="Look up employee ID, view profile and documents"
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          >
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontWeight: 900, fontSize: 12, color: "var(--wm-er-accent-hr)" }}>
                New to HR Management?
              </div>
      ...
      <div style={{ marginBottom: 12 }}>
        <HRStatsBar records={allRecords} />
      </div>
      <div style={{ marginBottom: 12 }}>
        <HRSearchBar value={searchQuery} onChange={setSearchQuery} />
      </div>
      ...
      {showFilters && (
        <div style={{ marginBottom: 12 }}>
          <HRFilterChips activeFilter={activeFilter} onChange={setActiveFilter} />
        </div>
      )}

      {searchQuery.length > 0 && (
        <div
          style={{ fontSize: 11, fontWeight: 700, color: "var(--wm-er-muted)", marginBottom: 8 }}
        >
      ...
      {filtered.length === 0 ? (
        searchQuery.length > 0 ? (
          ...
          <HREmptyState tab={activeTab as HRCandidateStatus} />
        ) : (
          <NoTeamMembers />
        )
      ) : (
