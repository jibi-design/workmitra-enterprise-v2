/** Job Mitra | EmployerFavoritesPage.tsx | C:\projects\WorkMitra_Enterprise_v2\src\features\employer\shiftJobs\pages\EmployerFavoritesPage.tsx */
...
function getSnapshot(): FavoriteWorker[] {
  ...
  if (raw !== _raw) {
    _raw = raw;
    _cache = favoritesStorage.getAll();
  }
  return _cache;
}
...
function InviteToShiftModal({
  ...
  const openPosts = useMemo(
    () => employerShiftStorage.getPosts().filter((p) => p.status === "active" && !p.isHiddenFromSearch),
    [],
  );
            ...
            {openPosts.length === 0 ? (
              ...
              <div style={{ display: "grid", gap: 8, maxHeight: 280, overflowY: "auto" }}>
                {openPosts.map((p) => {
                  ...
                  const dateStr = new Date(p.startAt).toLocaleDateString(undefined, {
                    month: "short",
                    day: "numeric",
                  });
...
export function EmployerFavoritesPage() {
  const favorites = useSyncExternalStore(favoritesStorage.subscribe, getSnapshot, getSnapshot);
  ...
  function handleAddManual() {
    const jmId = addInput.trim().toUpperCase();
    ...
    if (favoritesStorage.isFavorite(jmId)) {
      setAddError("This worker is already in your Favorites.");
      return;
    }

    favoritesStorage.addManual({ workerWmId: jmId, workerName: name });
    setAddInput("");
    setAddName("");
    setAddError("");
  ...
  function handleRemove(jmId: string) {
    favoritesStorage.remove(jmId);
    setRemovingId(null);
  }
  ...
  return (
    <div>
      {inviteTarget && (
        <InviteToShiftModal target={inviteTarget} onClose={() => setInviteTarget(null)} />
      )}
      ...
      <div className="wm-er-card" style={{ marginTop: 12 }}>
        ...
        {addError && (
          <div style={{ fontSize: 11, color: "var(--wm-error, #dc2626)", marginTop: 6 }}>{addError}</div>
        )}
        {addSuccess && (
      ...
      {favorites.length === 0 && (
        <div className="wm-er-card" style={{ marginTop: 12, padding: 28, textAlign: "center" }}>
          ...
          <div style={{ fontSize: 12, color: "var(--wm-er-muted)", marginTop: 6, lineHeight: 1.6 }}>
            Rate a worker and select &ldquo;Hire Again&rdquo; to automatically add them here. Or add by Job Mitra ID above.
          </div>
        </div>
      )}
        ...
        {filtered.map((fav) => (
          <div key={fav.id} className="wm-er-card" style={{ padding: 14 }}>
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
              ...
              <span
                style={{
                  ...
                  flexShrink: 0,
                  background:
                    fav.addedVia === "hire_again_rating" ? "rgba(22,163,74,0.08)" : "rgba(148,163,184,0.08)",
                  color: fav.addedVia === "hire_again_rating" ? "var(--wm-er-accent-shift, #16a34a)" : "var(--wm-er-muted)",
                  border:
                    fav.addedVia === "hire_again_rating"
            ...
            <div style={{ marginTop: 8, display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
              ...
              {fav.jobTitle && <span style={{ fontSize: 11, color: "var(--wm-er-muted)" }}>{fav.jobTitle}</span>}
            </div>
            ...
            <div style={{ marginTop: 10, display: "flex", gap: 8, justifyContent: "flex-end", flexWrap: "wrap" }}>
              ...
              {removingId === fav.workerWmId ? (
                <>
                  ...
                  <button
                    type="button"
                    onClick={() => handleRemove(fav.workerWmId)}
                    style={{
                      fontSize: 11,
                      fontWeight: 700,
                      padding: "4px 10px",
                ...
                <button
                  type="button"
                  onClick={() => setRemovingId(fav.workerWmId)}
                  style={{
                    fontSize: 11,
                    fontWeight: 600,
                    padding: "4px 10px",
