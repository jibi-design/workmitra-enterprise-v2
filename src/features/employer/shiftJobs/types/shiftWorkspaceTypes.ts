// App name: Job Mitra
// File name: shiftWorkspaceTypes.ts
// Full file path: C:\projects\WorkMitra_Enterprise_v2\src\features\employer\shiftJobs\types\shiftWorkspaceTypes.ts
...
export function isReadOnlyStatus(s: ShiftWorkspaceStatus): boolean {
  return s === "left" || s === "replaced" || s === "completed" || s === "cancelled";
}
...
export function updateRowStyle(u: ShiftWorkspaceUpdate): { border: string; bg: string } {
  if (u.kind === "direct") {
    return { border: "1px solid rgba(255,60,90,0.35)", bg: "rgba(255,60,90,0.08)" };
  }
