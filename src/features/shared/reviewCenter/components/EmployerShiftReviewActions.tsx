// App name: Job Mitra
// File name: EmployerShiftReviewActions.tsx
// Full file path: C:\projects\WorkMitra_Enterprise_v2\src\features\shared\reviewCenter\components\EmployerShiftReviewActions.tsx
...
import type { EmployerShiftReviewItem } from "../adapters/employerShiftReviewCenter.adapter";

type EmployerShiftReviewActionsProps = {
  items: EmployerShiftReviewItem[];
};

export function EmployerShiftReviewActions({ items }: EmployerShiftReviewActionsProps) {
      ...
      {items.map((item) => {
        ...
        return (
          ...
          >
            ...
            >
              Rate the worker and optionally request the worker to review this completed shift.
            </div>
            ...
            <div style={{ marginTop: 12, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              <button
                type="button"
                onClick={() =>
                  nav(ROUTE_PATHS.employerShiftWorkspace.replace(":workspaceId", item.workspaceId))
                }
                style={{
                  padding: "10px 10px",
                  borderRadius: 12,
              ...
              >
                {requestSent ? "Request Sent" : "Request Review"}
              </button>
            </div>
          </article>
        );
