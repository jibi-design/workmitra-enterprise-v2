  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
        ...
        <div style={{ fontSize: 14, fontWeight: 700, color: "var(--wm-emp-text)" }}>Your Job Mitra ID</div>
      </div>
      ...
      {hasId ? (
        ...
        <>
          ...
          <div style={{ fontSize: 12, color: "var(--wm-emp-muted)", lineHeight: 1.7, marginBottom: 6 }}>
            Your Job Mitra ID is created from your real name. Use your real, full name — not a nickname or fake name. Once created, your name and ID cannot be changed.
          </div>
...
export function EmployeeProfilePage() {
  ...
  const copyId = useCallback(() => {
    ...
    setNotice({ title: "Copied!", message: "Your Job Mitra ID has been copied to clipboard.", tone: "success" });
    ...
    <div>
      <div className="wm-pageHead">
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexShrink: 0 }}>
          {!isEditing ? (
            <button className="wm-primarybtn" type="button" onClick={() => setIsEditing(true)} style={{ fontSize: 12, padding: "6px 14px" }}>
              Edit
            </button>
          ) : (
            <>
              <button className="wm-outlineBtn" type="button" onClick={cancelChanges} style={{ fontSize: 12, padding: "6px 14px" }}>
                Cancel
              </button>
              <button className="wm-primarybtn" type="button" onClick={saveNow} disabled={!isDirty} style={{ fontSize: 12, padding: "6px 14px" }}>
                Save
              </button>
            </>
          )}
