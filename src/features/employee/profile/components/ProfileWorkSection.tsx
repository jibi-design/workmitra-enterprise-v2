...
export function ProfileWorkSection({ draft, disabled, onUpdate, sectionRef }: Props) {
  ...
  function removeSkill(s: string): void {
    onUpdate(
      "skills",
      draft.skills.filter((x) => x.toLowerCase() !== s.toLowerCase()),
    );
  }
  ...
  function removeLanguage(lang: string): void {
    onUpdate(
      "languages",
      draft.languages.filter((x) => x.toLowerCase() !== lang.toLowerCase()),
    );
  }
  ...
  return (
    <section className="wm-profileSectionCard" style={{ marginTop: 12 }} ref={sectionRef}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        <div className="wm-field">
          <label className="wm-label">
            Experience level <span style={{ color: "var(--wm-error)" }}>*</span>
          </label>
          <select
            className="wm-input"
        ...
        <div className="wm-field">
          <label className="wm-label">
            Preferred job types <span style={{ color: "var(--wm-error)" }}>*</span>
          </label>
          <div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 4 }}>
            ...
            >
              <input
                type="checkbox"
                checked={draft.preferShiftJobs}
                disabled={disabled}
                onChange={(e) => onUpdate("preferShiftJobs", e.target.checked)}
              />{" "}
              Shift Jobs
            </label>
            <label
            ...
            >
              <input
                type="checkbox"
                checked={draft.preferCareerJobs}
                disabled={disabled}
                onChange={(e) => onUpdate("preferCareerJobs", e.target.checked)}
              />{" "}
              Career Jobs
            </label>
          </div>
      ...
      <div className="wm-field" style={{ marginTop: 12 }}>
        <label className="wm-label">
          Skills <span style={{ color: "var(--wm-error)" }}>*</span>
        </label>
        {!disabled && (
          <div style={{ display: "flex", gap: 8 }}>
            ...
            />
            <button className="wm-primarybtn" type="button" onClick={() => addSkill(skillInput)}>
              Add
            </button>
          </div>
        )}
      ...
      <div className="wm-field" style={{ marginTop: 12 }}>
        <label className="wm-label">
          Languages <span style={{ color: "var(--wm-error)" }}>*</span>
        </label>
        {!disabled && (
          <div style={{ display: "flex", gap: 8 }}>
            <input
      ...
      <div className="wm-field" style={{ marginTop: 12 }}>
        <label className="wm-label">
          Availability (for shifts) <span style={{ color: "var(--wm-error)" }}>*</span>
        </label>
        <div style={{ fontSize: 11, color: "var(--wm-emp-muted)", marginBottom: 8 }}>
          Pick at least one day and one time.
        </div>

        <div
          style={{ fontSize: 12, fontWeight: 700, color: "var(--wm-emp-muted)", marginBottom: 6 }}
        >
          Days
        </div>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          {(["weekdays", "weekends"] as const).map((key) => (
            ...
            >
              <input
                type="checkbox"
                checked={draft.availability[key]}
                disabled={disabled}
                onChange={(e) =>
                  onUpdate("availability", { ...draft.availability, [key]: e.target.checked })
                }
              />
