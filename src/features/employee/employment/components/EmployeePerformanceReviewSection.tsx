...
import { usePerformanceReviews } from "../../../shared/hr/hrPublic";
import { performanceReviewStorage } from "../../../shared/hr/hrPublic";
import { REVIEW_TYPE_LABELS, RATING_LABELS, RATING_COLORS } from "../../../shared/hr/hrPublic";
import type { ReviewRating } from "../../../shared/hr/hrPublic";
    ...
    <div style={{ padding: 8, borderRadius: 6, background: "#fff" }}>
      ...
      <div
        style={{
          fontSize: 12,
          fontWeight: 600,
          color: color || "var(--wm-emp-text, var(--wm-er-text))",
          marginTop: 2,
          whiteSpace: "pre-wrap",
          lineHeight: 1.5,
        }}
            ...
            >
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <div style={{ display: "flex", gap: 1 }}>
                      {[1, 2, 3, 4, 5].map((s) => (
                        <span
                          key={s}
                          style={{ fontSize: 12, color: s <= rev.rating ? rColor : "#e5e7eb" }}
                        >
                          ★
                        </span>
                      ))}
              ...
              {isExpanded && (
                <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 8 }}>
                  <EmpRevBlock
                    label="Rating"
                    value={`${rev.rating}/5 — ${RATING_LABELS[rev.rating as ReviewRating]}`}
                    color={rColor}
                  />
                  <EmpRevBlock label="Strengths" value={rev.strengths} />
                  {rev.improvements && (
                    <EmpRevBlock label="Areas for Improvement" value={rev.improvements} />
                  )}
                  {rev.goalsNextPeriod && (
                    <EmpRevBlock label="Goals for Next Period" value={rev.goalsNextPeriod} />
                  )}
                  <EmpRevBlock label="Overall Comments" value={rev.overallComments} />
                  {rev.disputeReason && (
                    <EmpRevBlock
                      label="Your Dispute Reason"
                      value={rev.disputeReason}
                      color="#dc2626"
                    />
                  )}
