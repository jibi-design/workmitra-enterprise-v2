...
import { leaveManagementStorage } from "../../../shared/hr/hrPublic";
import { hrService } from "../../../shared/hr/hrPublic";
import type { LeaveType } from "../../../shared/hr/hrPublic";
import { LEAVE_TYPE_LABELS } from "../../../shared/hr/hrPublic";
...
export function LeaveApplyModal({
  ...
  const isValid =
    fromDateStr.length > 0 && toDateStr.length > 0 && toTs >= fromTs && reason.trim().length > 0;
      ...
      <div style={{ padding: 20 }}>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
          ...
          <div
            style={{
              ...
              height: 3,
              borderRadius: 999,
              background: step === "confirm" ? "#2563eb" : "var(--wm-er-border, #e5e7eb)",
            }}
          />
        </div>
        ...
        {step === "form" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div>
              ...
              {currentBalance && leaveType !== "unpaid" && (
                <div style={{ fontSize: 11, color: "var(--wm-er-muted)", marginTop: 3 }}>
                  Balance: {currentBalance.remaining} days remaining ({currentBalance.used} used of{" "}
            ...
            {totalDays > 0 && (
              <div
                style={{ fontSize: 12, fontWeight: 800, color: "var(--wm-er-text)", marginTop: -8 }}
              >
        ...
        {step === "confirm" && (
          <div>
            ...
            >
              <div>
                <strong>Type:</strong> {LEAVE_TYPE_LABELS[leaveType]}
              </div>
              <div>
                <strong>From:</strong> {formatDateDisplay(fromTs)}
              </div>
              <div>
                <strong>To:</strong> {formatDateDisplay(toTs)}
              </div>
              <div>
            ...
            <div style={{ marginTop: 14, display: "flex", justifyContent: "flex-end", gap: 10 }}>
              <button className="wm-outlineBtn" type="button" onClick={() => setStep("form")}>
                ← Edit
              </button>
