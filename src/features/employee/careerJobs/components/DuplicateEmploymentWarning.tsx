...
export function DuplicateEmploymentWarning() {
  ...
  const currentJob = activeRecords.find((r) => r.status === "working" || r.status === "notice");
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 12, color: AMBER, fontWeight: 600, lineHeight: 1.5 }}>
        You are currently working at {company}. Accepting a new offer will not end your current job
        automatically.
      </div>
    </div>
  );
