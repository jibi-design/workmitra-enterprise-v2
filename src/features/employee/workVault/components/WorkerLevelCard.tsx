...
// File: WorkerLevelCard.tsx
// Path: C:\projects\WorkMitra_Enterprise_v2\src\features\employee\workVault\components\WorkerLevelCard.tsx
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ fontSize: 16, fontWeight: 700, color }}>{threshold.label}</div>
            <div style={{ fontSize: 11, color, opacity: 0.8, marginTop: 1 }}>{threshold.badge}</div>
          </div>
        </div>

        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: 22, fontWeight: 700, color, lineHeight: 1 }}>{points.total}</div>
          <div style={{ fontSize: 11, color, opacity: 0.7, marginTop: 1 }}>points</div>
        </div>
      </div>
      ...
      {next && (
        <div style={{ marginTop: 12 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
            ...
            <span style={{ fontSize: 11, fontWeight: 600, color }}>{needed} pts needed</span>
          </div>
      ...
      {!next && (
        ...
        >
          Maximum level reached - Top Pick always
        </div>
      )}
      ...
      {points.total === 0 && recentHistory.length === 0 && (
        ...
        >
          Complete eligible work and build trusted feedback to earn points and level up.
        </div>
      )}
    </div>
  );
