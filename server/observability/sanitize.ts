/**
 * Defense Layer 2 — PII / secret redaction for logs and error reports.
 * Never log raw passwords, tokens, MUIDs, DB URLs, or session hashes.
 */
...
const SENSITIVE_KEY_RE =
  /^(password|passwd|pwd|passphrase|secret|token|access[_-]?token|refresh[_-]?token|id[_-]?token|invite[_-]?token|server[_-]?invite[_-]?token|csrf|csrf[_-]?token|authorization|cookie|set-cookie|session|session[_-]?hash|session[_-]?id|pepper|api[_-]?key|private[_-]?key|fcm|fcm[_-]?token|fcm[_-]?server[_-]?key|device[_-]?token|agora|agora[_-]?token|rtc[_-]?token|rtm[_-]?token|app[_-]?certificate|database[_-]?url|db[_-]?url|connection[_-]?string|supabase[_-]?(service[_-]?role|anon)[_-]?key|twilio[_-]?auth[_-]?token|wm[_-]?session|bearer)$/i;

/** Keys that identify worker / Mitra IDs — redact values (MUID leakage). */
const IDENTITY_KEY_RE =
  /^(muid|worker[_-]?wm[_-]?id|worker[_-]?ml[_-]?id|unique[_-]?id|jobmitra[_-]?user[_-]?id)$/i;

/** Inline secret patterns in free-form strings. */
const INLINE_PATTERNS: RegExp[] = [
  ...
  /** Public Mitra IDs in free text (structured keys handled separately). */
  /\bWM-[A-Z0-9-]{4,}\b/gi,
];
...
/**
 * Deep-sanitize any JSON-like value for logging / Sentry extras.
 */
