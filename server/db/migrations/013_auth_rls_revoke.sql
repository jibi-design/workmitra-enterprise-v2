...
-- public.auth_* : RLS + REVOKE so PostgREST Data API cannot read password hashes / PII.
-- File: server/db/migrations/013_auth_rls_revoke.sql
-- Note: no outer BEGIN/COMMIT — migrate.ts wraps each file in a transaction.
...
COMMENT ON TABLE public.auth_users IS
  'Sprint1: RLS enabled + anon/authenticated revoked. Access via server DATABASE_URL / service_role.';
